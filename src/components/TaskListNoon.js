...
export default function TaskListNoon(props) {
    ...
    return (
        <div className="mt-2">
            <h1 className="text-xs font-bold mt-4 text-gray-300 text-center">––– Noon –––</h1>
            <div className="justify-center max-w-full w-full grid grid-cols-1 grid-flow-row-dense gap-2">
                    ...
                    {activities.length > 0 ? (
                        ...
                        .filter(record => record.fields["time-of-day"].includes("Noon") === true)
                        .map((record) => (
                            <ActivityCard
